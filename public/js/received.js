document.addEventListener('DOMContentLoaded', () => {
    ...
    sendDataButton.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('http://localhost:3000/received', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    // Fetch and display received materials by date
    const fetchReceivedByDate = async (date) => {
        try {
            const response = await fetch(`http://localhost:3000/received/date/${date}`);
    ...
    // Set current date and fetch received materials
    ...
    materialsTable.addEventListener('click', async (event) => {
        if (event.target.classList.contains('delete-btn')) {
            const id = event.target.dataset.id;
            try {
                const response = await fetch(`http://localhost:3000/received/${id}`, {
                    method: 'DELETE'
                });
