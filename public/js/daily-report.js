document.addEventListener('DOMContentLoaded', () => {
    ...
    filterDateInput.value = currentDate; // Set filter date input to current date
    ...
    const populateMaterialList = async () => {
        try {
            ...
            materials.forEach(material => {
                ...
                option.value = material.materialName; // Set value to material name
                option.dataset.unit = material.unit; // Store unit as a data attribute
    ...
    // Handle material selection to update unit
    materialNameInput.addEventListener('input', () => {
        ...
        selectedUnit = selectedOption ? selectedOption.dataset.unit : '';  // Get unit from the selected option
    ...
    // Validate material name input on blur
    materialNameInput.addEventListener('blur', () => {
        ...
        if (!isValid) {
            materialNameInput.value = ''; // Clear invalid input
        }
    });
    ...
    saveButton.addEventListener('click', async () => {
        ...
        const data = {
            ...
            unit,
            notes,
            date: new Date(date).toISOString().split('T')[0] // Ensure date is correctly formatted
        };
        ...
        try {
            ...
            if (saveButton.dataset.id) {
                ...
                response = await fetch(`http://localhost:3000/daily-reports/${id}`, {
                    method: 'PUT',
                ...
                // Add new report
                response = await fetch('http://localhost:3000/daily-reports', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    sendDataButton.addEventListener('click', async () => {
        ...
        const dataToSend = Array.from(rows).map(row => {
            ...
            return {
                ...
                date: dateInput.value // Store the selected date
            };
        });
        ...
        try {
            const response = await fetch('http://localhost:3000/daily-reports', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const fetchDailyReportsByDate = async (date) => {
        try {
            const response = await fetch(`http://localhost:3000/daily-reports/date/${date}`);
    ...
    // Set current date and fetch daily reports
    ...
    materialsTable.addEventListener('click', async (event) => {
        if (event.target.classList.contains('edit-btn')) {
            ...
            selectedUnit = unit; // Ensure the selected unit is set correctly
    ...
    materialsTable.addEventListener('click', async (event) => {
        if (event.target.classList.contains('delete-btn')) {
            const id = event.target.dataset.id;
            try {
                const response = await fetch(`http://localhost:3000/daily-reports/${id}`, {
                    method: 'DELETE'
                });
